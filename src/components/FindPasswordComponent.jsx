...
import {findPasswordByEmail, sendFindPasswordEmail} from '../api/UserApi.jsx';
...
const FindPasswordComponent = () => {
  ...
  const handleCheckEmail = async (e) => {
    ...
    try {
      await validateEmail()
      const response = sendFindPasswordEmail({
        email
      })
      let codeValid = false;
      while (!codeValid) {
        try {
          ...
          const verificationResponse = await findPasswordByEmail({email, code})
